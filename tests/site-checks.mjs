...
const siteBase=canonical.replace(/\/$/,'');
const basePath=new URL(canonical).pathname;
function localPath(url){
  const pathname=new URL(url).pathname;
  assert.ok(pathname.startsWith(basePath),'Asset must stay within the project site');
  return pathname.slice(basePath.length);
}
for(const [path,url] of [['index.html',`${siteBase}/`],['about/index.html',`${siteBase}/about/`]]){
  ...
  for(const [,href] of html.matchAll(/(?:href|src)="([^"]+)"/g)){
    ...
    const resolved=localPath(new URL(href,url));
  ...
  const png=await readFile(join('dist',localPath(share)));
...
assert.ok(sitemap.includes(`<loc>${siteBase}/</loc>`));
